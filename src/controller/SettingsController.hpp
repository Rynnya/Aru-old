...
private:
	...
	std::shared_ptr<SettingsController::OutgoingResponse> updatePref(Int32 id, int32_t fav_mode, bool fav_relax, int32_t play_style) const;
	std::shared_ptr<SettingsController::OutgoingResponse> updateScoreboard(Int32 id, int32_t pref, int32_t auto_classic, int32_t auto_relax) const;
public:
		...
		PATH(Int32, id), AUTHORIZATION(std::shared_ptr<TokenObject>, authObject), REQUEST(std::shared_ptr<IncomingRequest>, request))
	{
		...
		if (config::avatar_folder == "/path/to/folder/{}.png")
		{
			...
			return createResponse(Status::CODE_422, aru::createError(Status::CODE_422, "Sorry, but avatars currently not available to change!").c_str());
		}
		...
		PATH(Int32, id), AUTHORIZATION(std::shared_ptr<TokenObject>, authObject), BODY_STRING(String, fav_modes))
	{
		...
		if (!body.is_discarded())
		{
			if (body["favourite_mode"].is_null() || !body["favourite_mode"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: favourite_mode").c_str());
			if (body["favourite_relax"].is_null() || !body["favourite_relax"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: favourite_relax").c_str());
			if (body["play_style"].is_null() || !body["play_style"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: play_style").c_str());

			return updatePref(id, body["favourite_mode"], aru::utils::intToBoolean(body["favourite_relax"]), body["play_style"]);
		}
		...
		PATH(Int32, id), AUTHORIZATION(std::shared_ptr<TokenObject>, authObject), BODY_STRING(String, scoreboard))
	{
		...
		if (!body.is_discarded())
		{
			if (body["preferences"].is_null() || !body["preferences"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: preferences").c_str());

			if (body["auto_classic"].is_null() || !body["auto_classic"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: auto_classic").c_str());

			if (body["auto_relax"].is_null() || !body["auto_relax"].is_number_integer())
				return createResponse(Status::CODE_400, aru::createError(Status::CODE_400, "No data provided: auto_relax").c_str());
