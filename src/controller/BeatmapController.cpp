...
{
	beatmaps b_table{};
	config::osu_api::currently_running = true;
	auto db(himitsu::ConnectionPool::getInstance()->getConnection());

	for (const int id : beatmaps_v)
	{
		auto [unused1, std_output]   = himitsu::curl::get("https://old.ppy.sh/api/get_beatmaps?k=" + config::osu_api::osu_key + "&a=1&m=0&b=" + fmt::to_string(id));
		auto [unused2, taiko_output] = himitsu::curl::get("https://old.ppy.sh/api/get_beatmaps?k=" + config::osu_api::osu_key + "&a=1&m=1&b=" + fmt::to_string(id));
		auto [unused3, ctb_output]   = himitsu::curl::get("https://old.ppy.sh/api/get_beatmaps?k=" + config::osu_api::osu_key + "&a=1&m=2&b=" + fmt::to_string(id));
		auto [unused4, mania_output] = himitsu::curl::get("https://old.ppy.sh/api/get_beatmaps?k=" + config::osu_api::osu_key + "&a=1&m=3&b=" + fmt::to_string(id));
		...
		std::this_thread::sleep_for(std::chrono::seconds(config::osu_api::cooldown));
	}

	config::osu_api::currently_running = false;
}